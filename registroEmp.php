<html>
    <body>
        ...
        if ($conexao->connect_errno) {
            ...
            $inicioEmp = $conexao->real_escape_string($_POST['inicioEmp']);
            $fimEmp = $conexao->real_escape_string($_POST['fimEmp']);
            $renovado = $conexao->real_escape_string($_POST['renovado']);

            // Insere o novo empréstimo no banco de dados
            $sqlInserirEmprestimo = "INSERT INTO `emprestimo`
                                        (`titulo`, `nome`, `inicioEmp`, `fimEmp`, `renovado`)
                                    VALUES
                                        ('$titulo', '$nome', '$inicioEmp', '$fimEmp', '0')";
