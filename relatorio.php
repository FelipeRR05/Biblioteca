    ...
    <nav class="navbar navbar-expand-lg bg-white navbar-light shadow sticky-top p-0">
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav ms-auto p-4 p-lg-0">
                ...
                <a href="relatorio.php" class="nav-item nav-link active">Empréstimos</a>
    ...
    <div class="container-xxl py-5 category">
        <div class="container">
            <div class="text-center wow fadeInUp" data-wow-delay="0.1s">
                ...
                <h1 class="mb-5">Gerenciar Empréstimos</h1>
            </div>
            <div class="row g-4">
...
if (isset($_GET['search'])) {
    ...
    $sql = "SELECT * FROM emprestimo WHERE nome LIKE '%$search%' OR titulo LIKE '%$search%' ORDER BY nome";
} else {
    // Consulta para selecionar todos os livros do acervo
    $sql = "SELECT * FROM emprestimo ORDER BY titulo";
}

$result = $conn->query($sql);
echo '
<div class="container">
    <h2 style="margin-bottom: 20px;">Lista de ' . $result->num_rows . ' Empréstimos</h2>

    <!-- Barra de pesquisa com ícone de lupa -->
    <form method="GET" action="relatorio.php">
        <div style="display: flex; align-items: center;">
            <input type="text" name="search" placeholder="Pesquisar empréstimo">
    ...
    <ul>';
        if ($result->num_rows > 0) {
            while ($livro = $result->fetch_assoc()) {
                $empId = $livro["id"]; // Corrigido de $row para $livro
                $titulo = $livro['titulo'];
                $nome = $livro['nome'];
                ...
                echo '<span style="margin-right: 10px;">- Início do Empréstimo: ' . $inicioEmp . '</span>';
                echo '<span style="margin-right: 10px;">- Fim do Empréstimo: ' . $fimEmp . '</span>';
                echo '<a href="atualizarEmp.php?id=' . $empId . '"><i class="fas fa-edit"></i></a>';
                echo '<a href="devolucao.php?id=' . $empId . '" onclick="return confirm(\'Tem certeza de que deseja confirmar a devolução do livro?\')" style="margin-left: 10px;"><i class="fas fa-check-circle"></i></a>';
                echo '<a href="deletarEmp.php?id=' . $empId . '" onclick="return confirm(\'Tem certeza de que deseja deletar o empréstimo?\')" style="margin-left: 10px;"><i class="fas fa-trash"></i></a>';
                echo '</li>';
            }
        } else {
...
<div style="display: flex; justify-content: center; padding-bottom: 20px; padding-top: 15;">
    <form style="background-color: #f7f7f7; box-shadow: 0px 0px 15px #666; width: 55%; border-radius: 15px;" method="post" action="adicionarEmp.php" id="formlogin" name="formlogin">
        <div style="padding: 50px; display: flex; flex-wrap: wrap;">
            <div style="padding-bottom: 25px; width: 100%; display: block;">
                <h2 style="display: flex; justify-content: center; color: rgba(17, 27, 88, 0.7);">Adicionar Novo Empréstimo</h2>
            </div>
        ...
        ?>
            ...
            <div style="width: 100%; padding-bottom: 12px; display: flex;">
                <div style="flex: 1;">
                    ...
                    <input style="width: 100%; border: 2px solid #ccc; height: 55px; padding: 10px; margin: 10px; border-radius: 5px;" type="date" id="inicioEmp" name="inicioEmp" min="<?= date('Y-m-d') ?>" max="<?= date('Y-m-d', strtotime('+0 days')) ?>" required>
                </div>
                <div style="flex: 1; margin-left: 20px;">
                    <label style="float: left; padding-bottom: 5px;" for="fimEmp">Fim do Empréstimo</label><br>
                    <input style="width: 100%; border: 2px solid #ccc; height: 55px; padding: 10px; margin: 10px; border-radius: 5px;" type="date" id="fimEmp" name="fimEmp" min="<?= date('Y-m-d', strtotime('+1 day')) ?>" max="<?= date('Y-m-d', strtotime('+15 days')) ?>" required>
                </div>
            </div>
